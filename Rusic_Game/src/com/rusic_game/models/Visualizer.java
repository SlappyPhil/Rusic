...
public class Visualizer {
	...
	public Visualizer(World world, AudioAnalyzer analyzer) {
		...
		for(int i = 0; i < numBars; i++) {
			...
			if(i < numBars / 2) {
				groundBarDef.position.set(-(width * (numBars - i)/numBars - width/2), -height/2);
				ceilingBarDef.position.set(-(width * (numBars - i)/numBars - width/2), height/2);
			}
			else {
				groundBarDef.position.set(width * i/numBars - width/2, -height/2);
				ceilingBarDef.position.set(width * i/numBars - width/2, height/2);
			}
	...
	public void update() {
		...
		for(int i = 0; i < numBars; i++) {
			...
			PolygonShape shape = new PolygonShape();
            shape.setAsBox((width / numBars) / 2, scale(avg(histoX, nb)) / 2);
